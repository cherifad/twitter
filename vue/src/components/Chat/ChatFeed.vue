    ...
    <div class="flex overflow-scroll justify-between px-4 flex-col flex-1 max-h-full">
      ...
      >
        ...
        <div class="text-center">
          <RouterLink
            :to="`/${state.current_user.username}`"
            class="flex gap-2 items-center font-bold hover:underline"
            >{{ state.current_user.name }}
            <IconVerified v-if="state.current_user.premium" />
          </RouterLink>
          <span class="text-slate-700 dark:text-zinc-500"
            >@{{ state.current_user.username }}</span
          >
        </div>
        <div class="flex">
          <span
            class="text-slate-700 dark:text-zinc-500 gap-2 flex items-center"
          >
      ...
      <div class="pb-7 flex h-fit flex-col justify-end overflow-scroll">
        <SingleChat
          ...
          :fromUser="message.sender_id == userId"
        />
      </div>
    </div>
